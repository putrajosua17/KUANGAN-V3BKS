...
export default function App() {
  ...
  const handleExport = () => {
    ...
    const pajakRows = transactions.filter(t => t.type === "pajak_masuk").length
      ? transactions.filter(t => t.type === "pajak_masuk").map(t => ({
          Tanggal: t.date,
          Keterangan: t.entity || "",
          Kantong: t.method || "",
          Jumlah: t.amount,
      ...
      : [{ Catatan: "Belum ada pajak masuk yang dicatat" }];
  ...
  const breakEvenData = useMemo(() => {
    ...
    const hasDurationData = rentalTxs.some((t) => t.duration && t.duration > 0);
    const dailyAvg = daysElapsed > 0 ? rentalIncome / daysElapsed : 0;
    const projected = dailyAvg * totalDays;

    let status, statusColor, statusBg;
    if (projected < BREAKEVEN.targetBreakEven) {
      status = "RUGI"; statusColor = "#D1574A"; statusBg = "rgba(209,87,74,0.12)";
    } else if (projected < BREAKEVEN.targetHealthy) {
      status = "WASPADA"; statusColor = "#C9A227"; statusBg = "rgba(201,162,39,0.12)";
    } else if (projected < BREAKEVEN.targetStrong) {
      status = "SEHAT"; statusColor = "#4CAF61"; statusBg = "rgba(76,175,97,0.12)";
    } else {
      status = "KUAT"; statusColor = "#4D7FB0"; statusBg = "rgba(77,127,176,0.15)";
    }

    const gapBreakEven = Math.max(0, BREAKEVEN.targetBreakEven - projected);
    const gapHealthy = Math.max(0, BREAKEVEN.targetHealthy - projected);
    const hoursToBreakEven = gapBreakEven > 0 ? Math.ceil(gapBreakEven / BREAKEVEN.marginBlended) : 0;
    const hoursToBreakEvenWD = gapBreakEven > 0 ? Math.ceil(gapBreakEven / BREAKEVEN.marginWeekdayOffPeak) : 0;
    const hoursToBreakEvenWE = gapBreakEven > 0 ? Math.ceil(gapBreakEven / BREAKEVEN.marginWeekendOffPeak) : 0;
    const utilizationPct = BREAKEVEN.totalHoursPerMonth > 0
      ? Math.min(100, Math.round((hoursBooked / BREAKEVEN.totalHoursPerMonth) * 100))
      : 0;
    const progressToBreakEven = Math.min(100, Math.round((projected / BREAKEVEN.targetBreakEven) * 100));
    const progressToStrong = Math.min(100, Math.round((projected / BREAKEVEN.targetStrong) * 100));

    return {
      monthStr, year, month, daysElapsed, daysRemaining, totalDays,
      rentalIncome, hoursBooked, hasDurationData, dailyAvg, projected,
  ...
  // Pajak yang terkumpul (tidak masuk income, tapi masuk saldo)
  const pajakTerkumpulBulanIni = useMemo(() => {
    const now = new Date();
    const monthStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;
    return transactions
      .filter((t) => t.type === "pajak_masuk" && t.date.startsWith(monthStr))
      .reduce((sum, t) => sum + t.amount, 0);
  }, [transactions]);

  const pajakTerkumpulTotal = useMemo(
    () => transactions.filter((t) => t.type === "pajak_masuk").reduce((sum, t) => sum + t.amount, 0),
    [transactions]
  );
      ...
      <div className="px-4 py-5 md:px-6" style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div className="flex gap-2" style={{ marginBottom: "1.2rem" }}>
          ...
          <button
            ...
            style={{ flex: 1, borderRadius: 14, padding: "0.8rem 1rem", border: "1.5px solid rgba(201,162,39,0.25)" }}
          >
            <HandCoins size={16} className="v3-gold" />
            <span style={{ fontSize: "0.85rem", fontWeight: 600 }}>Piutang</span>
          </button>
        </div>
              ...
              .map((a) => {
                ...
                return (
                  <div
                    ...
                    style={{ background: bg, borderRadius: 12, padding: "0.75rem 0.9rem", borderLeft: `3px solid ${color}` }}
                  >
                    <Icon size={15} style={{ color, flexShrink: 0, marginTop: "0.1rem" }} />
                    <p style={{ fontSize: "0.82rem", flex: 1, lineHeight: 1.5 }}>{a.message}</p>
                    <button
        ...
        {/* Total Saldo + Per Kantong */}
        <div style={{ marginBottom: "1.4rem" }}>
          <div className="flex items-center justify-between" style={{ marginBottom: "0.75rem" }}>
            <p className="v3-muted" style={{ fontSize: "0.75rem", letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 600 }}>Saldo Real</p>
            <div className="flex gap-1.5">
              <select
                value={selectedYear}
                onChange={(e) => setSelectedYear(Number(e.target.value))}
                className="v3-input"
                style={{ borderRadius: 8, padding: "0.3rem 0.5rem", fontSize: "0.8rem" }}
              >
          ...
          <div className="v3-surface" style={{ borderRadius: 18, padding: "1.4rem 1.5rem", marginBottom: "0.75rem" }}>
            <p className="v3-muted" style={{ fontSize: "0.75rem", letterSpacing: "0.08em", textTransform: "uppercase" }}>
              Total{selectedMonth !== null ? " · " + MONTHS[selectedMonth] + " " + selectedYear : ""}
            </p>
            <p className="v3-mono v3-gold" style={{ fontSize: "2.2rem", fontWeight: 700, marginTop: "0.25rem", letterSpacing: "-0.01em" }}>
              {formatRupiah(totalSaldo)}
            </p>
            <div style={{ height: 1, background: "rgba(201,162,39,0.15)", margin: "0.75rem 0" }} />
            <p className="v3-muted" style={{ fontSize: "0.75rem", letterSpacing: "0.06em", textTransform: "uppercase" }}>
              Saldo Bersih
            </p>
            <p className={"v3-mono " + (saldoBersih >= 0 ? "v3-green" : "v3-red")} style={{ fontSize: "1.4rem", fontWeight: 700, marginTop: "0.2rem" }}>
              {formatRupiah(saldoBersih)}
            </p>
            {pajakTerkumpulBulanIni > 0 && (
              <div style={{ marginTop: "0.5rem", padding: "0.35rem 0.65rem", background: "rgba(77,127,176,0.13)", borderRadius: 7, display: "inline-flex", alignItems: "center", gap: "0.4rem" }}>
                <span style={{ fontSize: "0.67rem", color: "#4D7FB0", fontWeight: 600 }}>📋 Pajak bulan ini:</span>
                <span className="v3-mono" style={{ fontSize: "0.7rem", fontWeight: 700, color: "#4D7FB0" }}>{formatRupiah(pajakTerkumpulBulanIni)}</span>
              </div>
            )}
            <p className="v3-muted" style={{ fontSize: "0.73rem", marginTop: "0.4rem" }}>
              Disinkron {timeAgo(lastSynced)}
            </p>
          </div>
                ...
                <div key={m} className="v3-surface" style={{ borderRadius: 14, overflow: "hidden" }}>
                  ...
                  <div style={{ padding: "0.9rem 1rem" }}>
                    <div className="flex items-center gap-1.5" style={{ marginBottom: "0.5rem" }}>
                      ...
                      <span className="v3-muted" style={{ fontSize: "0.78rem", fontWeight: 600 }}>{m}</span>
                    </div>
                    <p className="v3-mono" style={{ fontSize: "1.1rem", fontWeight: 700 }}>
        ...
        <div className="v3-surface" style={{ borderRadius: 16, padding: "1.1rem 1.2rem", marginBottom: "1.4rem" }}>
          <div className="flex items-center justify-between" style={{ marginBottom: "0.7rem" }}>
            <div className="flex items-center gap-2">
              <Clock size={15} className="v3-gold" />
              <p className="v3-display" style={{ fontSize: "0.92rem", fontWeight: 700 }}>Hari Ini · {todayStr}</p>
            </div>
            <span className="v3-muted" style={{ fontSize: "0.75rem" }}>{todaySummary.txs.length} transaksi</span>
          </div>
            ...
            <>
              <div className="flex gap-2" style={{ marginBottom: "0.75rem", flexWrap: "wrap" }}>
                {todaySummary.income > 0 && (
                  <span className="v3-mono v3-surface-alt" style={{ fontSize: "0.78rem", padding: "0.3rem 0.75rem", borderRadius: 999 }}>
                ...
                {todaySummary.transfer > 0 && (
                  <span className="v3-mono v3-surface-alt" style={{ fontSize: "0.78rem", padding: "0.3rem 0.75rem", borderRadius: 999, color: "#8A9099" }}>
              ...
              <div className="flex flex-col" style={{ gap: "0.5rem" }}>
                {todaySummary.txs.slice(0, 6).map((t) => {
                  ...
                  return (
                    <div key={t.id} className="flex items-center justify-between v3-surface-alt" style={{ borderRadius: 10, padding: "0.5rem 0.75rem" }}>
                      <span style={{ fontSize: "0.8rem" }}>
                        <span style={{ fontWeight: 600 }}>{label}</span>
                        {t.entity ? <span className="v3-muted"> · {t.entity}</span> : ""}
                        {kantong ? <span className="v3-muted"> · {kantong}</span> : ""}
                      </span>
                      <span className="v3-mono" style={{ fontSize: "0.8rem", color, flexShrink: 0, marginLeft: "0.5rem", fontWeight: 700 }}>
                ...
                {todaySummary.txs.length > 6 && (
                  <button
                    ...
                    className="v3-muted"
                    style={{ fontSize: "0.78rem", textAlign: "center", marginTop: "0.3rem", padding: "0.4rem", borderRadius: 8, background: "rgba(255,255,255,0.04)" }}
                  >
        ...
        {/* ─── Piutang Aktif ─── */}
        <div className="v3-surface" style={{ borderRadius: 16, padding: "1.2rem 1.3rem", marginBottom: "1.4rem", marginTop: "1.4rem" }}>
          <div className="flex items-center justify-between" style={{ marginBottom: "0.8rem" }}>
            <div className="flex items-center gap-2">
              <AlertTriangle size={16} style={{ color: piutangList.length > 0 ? "#D1574A" : "#8A9099" }} />
              <p className="v3-display" style={{ fontSize: "1rem", fontWeight: 700, letterSpacing: "0.02em" }}>
                Piutang Aktif
              </p>
            </div>
            {piutangList.length > 0 && (
              <span className="v3-mono" style={{ fontSize: "0.85rem", fontWeight: 700, color: "#D1574A" }}>
          ...
          {piutangList.length === 0 ? (
            <div className="flex items-center gap-2" style={{ background: "rgba(76,175,97,0.1)", borderRadius: 12, padding: "0.75rem 0.9rem" }}>
              <CheckCircle2 size={15} className="v3-green" />
              <p style={{ fontSize: "0.82rem" }}>Semua kontrak sudah lunas.</p>
            </div>
          ) : (
            <div className="flex flex-col" style={{ gap: "0.75rem" }}>
              {piutangList.map((p) => (
                <div key={p.id} className="v3-surface-alt" style={{ borderRadius: 12, padding: "0.8rem 0.9rem", borderLeft: "3px solid #D1574A" }}>
                  <div className="flex items-start justify-between gap-2">
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: "0.88rem", fontWeight: 700 }}>{p.entity || p.category}</p>
                      <p className="v3-muted" style={{ fontSize: "0.74rem", marginTop: "0.2rem" }}>
                        {p.date} · {p.category} · {p.status}
                      </p>
                      <div className="flex gap-3" style={{ marginTop: "0.4rem", flexWrap: "wrap" }}>
                        <span className="v3-mono" style={{ fontSize: "0.75rem", color: "#8A9099" }}>
                          Masuk: <span style={{ color: "#4CAF61", fontWeight: 600 }}>{formatRupiah(p.amount)}</span>
                        </span>
                    ...
                    <button
                      ...
                      className="v3-gold-bg"
                      style={{ borderRadius: 10, padding: "0.45rem 0.85rem", fontSize: "0.78rem", fontWeight: 700, whiteSpace: "nowrap", flexShrink: 0 }}
                    >
                      Tandai Lunas
                    </button>
                  </div>
        ...
        {/* ─── Piutang Non-Bisnis ─── */}
        <div className="v3-surface" style={{ borderRadius: 16, padding: "1.2rem 1.3rem", marginBottom: "1.4rem" }}>
          <div className="flex items-center justify-between" style={{ marginBottom: "0.8rem" }}>
            <div className="flex items-center gap-2">
              <HandCoins size={16} style={{ color: totalPiutangNonBisnis > 0 ? "#C9A227" : "#8A9099" }} />
              <p className="v3-display" style={{ fontSize: "1rem", fontWeight: 700, letterSpacing: "0.02em" }}>
                Piutang Non-Bisnis
              </p>
            </div>
            {totalPiutangNonBisnis > 0 && (
              <span className="v3-mono v3-gold" style={{ fontSize: "0.85rem", fontWeight: 700 }}>
                {formatRupiah(totalPiutangNonBisnis)}
              </span>
            )}
          </div>
          <p className="v3-muted" style={{ fontSize: "0.78rem", marginBottom: "0.75rem" }}>
            Pinjaman ke owner/PT lain — tidak dihitung sebagai expense bisnis.
          </p>
          {piutangNonBisnisList.length === 0 ? (
            <div className="flex items-center gap-2" style={{ background: "rgba(76,175,97,0.1)", borderRadius: 12, padding: "0.75rem 0.9rem" }}>
              <CheckCircle2 size={15} className="v3-green" />
              <p style={{ fontSize: "0.82rem" }}>Tidak ada piutang non-bisnis aktif.</p>
            </div>
          ) : (
            <div className="flex flex-col" style={{ gap: "0.75rem" }}>
              {piutangNonBisnisList.map((e) => {
                ...
                return (
                  <div key={e.name} className="v3-surface-alt" style={{ borderRadius: 12, padding: "0.8rem 0.9rem", borderLeft: `3px solid ${lunas ? "#4CAF61" : "#C9A227"}` }}>
                    <div className="flex items-start justify-between gap-2">
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div className="flex items-center gap-1.5">
                          <p style={{ fontSize: "0.88rem", fontWeight: 700 }}>{e.name}</p>
                          {e.category && (
                            <span className="v3-muted" style={{ fontSize: "0.7rem", padding: "0.1rem 0.5rem", borderRadius: 999, background: "rgba(255,255,255,0.07)" }}>
                              {e.category}
                            </span>
                          )}
                        </div>
                        <div className="flex gap-3" style={{ marginTop: "0.35rem", flexWrap: "wrap" }}>
                          <span className="v3-mono" style={{ fontSize: "0.75rem", color: "#D1574A" }}>Keluar: {formatRupiah(e.keluar)}</span>
                          <span className="v3-mono" style={{ fontSize: "0.75rem", color: "#4CAF61" }}>Kembali: {formatRupiah(e.balik)}</span>
                          <span className="v3-mono" style={{ fontSize: "0.75rem", fontWeight: 700, color: lunas ? "#4CAF61" : "#C9A227" }}>Sisa: {formatRupiah(Math.max(0, e.sisa))}</span>
                        </div>
                      </div>
                      {!lunas && (
                        <button
                          ...
                          style={{ borderRadius: 10, padding: "0.4rem 0.75rem", fontSize: "0.75rem", fontWeight: 700, border: "1px solid rgba(201,162,39,0.4)", color: "#C9A227", whiteSpace: "nowrap", flexShrink: 0 }}
                        >
                          Catat Kembali
                        </button>
                      )}
        ...
        <div className="v3-surface" style={{ borderRadius: 16, padding: "1.2rem 1.3rem", marginBottom: "1.4rem" }}>
          ...
          {todayRecon ? (
            Math.round(todayRecon.totalDiff) === 0 ? (
              <div className="flex items-center gap-2" style={{ background: "rgba(76,175,97,0.12)", borderRadius: 12, padding: "0.75rem 0.9rem" }}>
                <CheckCircle2 size={16} className="v3-green" style={{ flexShrink: 0 }} />
                <p style={{ fontSize: "0.82rem" }}>Kas hari ini sudah cocok dengan sistem.</p>
              </div>
            ) : (
              <div className="flex items-center gap-2" style={{ background: "rgba(209,87,74,0.12)", borderRadius: 12, padding: "0.75rem 0.9rem" }}>
                <AlertTriangle size={16} className="v3-red" style={{ flexShrink: 0 }} />
                <p style={{ fontSize: "0.82rem" }}>
                  Ada selisih: <span className="v3-mono v3-red" style={{ fontWeight: 700 }}>{formatRupiah(todayRecon.totalDiff)}</span>
                </p>
              </div>
            )
          ) : (
            <div className="flex items-center gap-2" style={{ background: "rgba(201,162,39,0.12)", borderRadius: 12, padding: "0.75rem 0.9rem" }}>
              <AlertTriangle size={16} className="v3-gold" style={{ flexShrink: 0 }} />
              <p style={{ fontSize: "0.82rem" }}>Hari ini ({todayStr}) belum direkonsiliasi.</p>
            </div>
          )}

          <p className="v3-muted" style={{ fontSize: "0.75rem", marginTop: "0.7rem" }}>
            {reconStats.total} kali dicek · {reconStats.withDiff} kali ada selisih tahun {selectedYear}
          </p>

          {yearReconciliations.length > 0 && (
            <div className="flex flex-col" style={{ gap: "0.6rem", marginTop: "0.9rem" }}>
              {yearReconciliations.slice(0, 5).map((r) => {
                const ok = Math.round(r.totalDiff) === 0;
                return (
                  <div key={r.id} className="v3-surface-alt flex items-center justify-between" style={{ borderRadius: 12, padding: "0.65rem 0.85rem" }}>
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontSize: "0.82rem", fontWeight: 600 }}>{r.date}</p>
                      <p className="v3-muted" style={{ fontSize: "0.72rem", marginTop: "0.15rem" }}>
                        {METHODS.map((m) => `${m} ${r.perMethod[m].diff === 0 ? "0" : formatRupiah(r.perMethod[m].diff)}`).join(" · ")}
                      </p>
                    </div>
                    <div className="flex items-center gap-2" style={{ flexShrink: 0 }}>
                      <span className={"v3-mono " + (ok ? "v3-green" : "v3-red")} style={{ fontSize: "0.85rem", fontWeight: 700 }}>
                        {formatRupiah(r.totalDiff)}
                      </span>
                      <button onClick={() => setConfirmDelete({ type: "reconciliation", id: r.id })} aria-label="Hapus">
        ...
        {missingRecurring.length > 0 && (
          <div className="v3-surface" style={{ borderRadius: 16, padding: "1.2rem 1.3rem", marginBottom: "1.4rem" }}>
            <div className="flex items-center gap-2" style={{ marginBottom: "0.6rem" }}>
              <AlertTriangle size={16} className="v3-gold" />
              <p className="v3-display" style={{ fontSize: "1rem", fontWeight: 700 }}>Pengingat Biaya Rutin</p>
            </div>
            <p style={{ fontSize: "0.82rem" }}>
              Belum tercatat bulan ini: <span style={{ fontWeight: 600 }}>{missingRecurring.join(", ")}</span>
            </p>
            <p className="v3-muted" style={{ fontSize: "0.75rem", marginTop: "0.3rem" }}>
        ...
        <div className="v3-surface" style={{ borderRadius: 14, padding: "0.8rem 0.9rem", marginBottom: "0.9rem" }}>
          ...
          <div className="v3-scroll flex gap-1.5" style={{ overflowX: "auto", marginBottom: "0.5rem" }}>
            {[["all", "Semua"], ["income", "Income"], ["expense", "Expense"], ["transfer", "Transfer"], ["prive", "Prive"], ["piutang_keluar", "Piutang Keluar"], ["piutang_balik", "Pengembalian"], ["pajak_masuk", "Pajak Masuk"]].map(([val, label]) => (
              <button
                key={val}
...
function BreakEvenCard({ data }) {
  ...
  const milestones = [
    { label: "Break-Even", value: BREAKEVEN.targetBreakEven, color: "#D1574A" },
    { label: "Sehat", value: BREAKEVEN.targetHealthy, color: "#C9A227" },
    { label: "Kuat", value: BREAKEVEN.targetStrong, color: "#4D7FB0" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", marginBottom: "1.4rem" }}>
      ...
      <div
        className="v3-surface"
        style={{ borderRadius: 18, padding: "1.2rem 1.4rem", borderLeft: `5px solid ${statusColor}` }}
      >
        <div className="flex items-center justify-between" style={{ marginBottom: "0.5rem" }}>
          <div>
            <p className="v3-muted" style={{ fontSize: "0.65rem", letterSpacing: "0.12em", textTransform: "uppercase" }}>
              Break-Even Tracker · Rental Bulan Ini
            </p>
            <div className="flex items-center gap-2" style={{ marginTop: "0.2rem" }}>
              <span
                className="v3-mono"
                style={{ fontSize: "0.7rem", fontWeight: 800, background: statusColor, color: "#0B0D10", borderRadius: 6, padding: "0.15rem 0.6rem", letterSpacing: "0.08em" }}
              >
                {status}
              </span>
              <p className="v3-muted" style={{ fontSize: "0.72rem" }}>
                Hari ke-{daysElapsed} dari {totalDays} · {daysRemaining} hari tersisa
              </p>
            </div>
          </div>
          <div style={{ textAlign: "right" }}>
            <p className="v3-muted" style={{ fontSize: "0.65rem" }}>Proyeksi akhir bulan</p>
            <p className="v3-mono" style={{ fontSize: "1.15rem", fontWeight: 700, color: statusColor }}>
              {formatRupiah(Math.round(projected))}
            </p>
          </div>
        </div>
        ...
        <div style={{ marginBottom: "0.6rem" }}>
          <div className="flex items-center justify-between" style={{ marginBottom: "0.25rem" }}>
            <span className="v3-muted" style={{ fontSize: "0.68rem" }}>Rental aktual: {formatRupiah(rentalIncome)}</span>
            <span className="v3-muted" style={{ fontSize: "0.68rem" }}>Rata-rata: {formatRupiah(Math.round(dailyAvg))}/hari</span>
          </div>
          <div style={{ position: "relative", height: 10, background: "rgba(255,255,255,0.07)", borderRadius: 5 }}>
            <div
              style={{
                position: "absolute", left: 0, top: 0, height: 10,
                width: progressToStrong + "%",
                background: `linear-gradient(90deg, ${statusColor}, ${statusColor}cc)`,
                borderRadius: 5, transition: "width 0.6s ease",
              }}
            />
            {milestones.map((m) => {
              const pct = Math.min(100, (m.value / BREAKEVEN.targetStrong) * 100);
              return (
                <div
                  key={m.label}
                  style={{
                    ...
                    background: m.color, borderRadius: 1, opacity: 0.8,
                  }}
                />
              );
            })}
          </div>
          <div className="flex justify-between" style={{ marginTop: "0.2rem" }}>
            {milestones.map((m) => (
              <span key={m.label} style={{ fontSize: "0.6rem", color: m.color, fontWeight: 600 }}>
                {m.label} {formatRupiah(m.value / 1000000)}jt
              </span>
            ))}
          </div>
        </div>
      ...
      {/* Gap Calculator */}
      {gapBreakEven > 0 ? (
        <div className="v3-surface" style={{ borderRadius: 14, padding: "1rem 1.1rem", borderLeft: "4px solid #D1574A" }}>
          <p className="v3-display" style={{ fontSize: "0.85rem", fontWeight: 600, marginBottom: "0.6rem" }}>
            ⚡ Gap ke Break-Even
          </p>
          <div className="grid grid-cols-2 gap-2" style={{ marginBottom: "0.7rem" }}>
            <div className="v3-surface-alt" style={{ borderRadius: 10, padding: "0.6rem 0.75rem" }}>
              <p className="v3-muted" style={{ fontSize: "0.65rem" }}>Kekurangan proyeksi</p>
              <p className="v3-mono v3-red" style={{ fontSize: "1rem", fontWeight: 700 }}>{formatRupiah(gapBreakEven)}</p>
            </div>
            <div className="v3-surface-alt" style={{ borderRadius: 10, padding: "0.6rem 0.75rem" }}>
              <p className="v3-muted" style={{ fontSize: "0.65rem" }}>Jam butuh diisi (blended)</p>
              <p className="v3-mono v3-gold" style={{ fontSize: "1rem", fontWeight: 700 }}>{hoursToBreakEven} jam</p>
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between" style={{ background: "rgba(201,162,39,0.08)", borderRadius: 8, padding: "0.45rem 0.65rem" }}>
              <div>
                <p style={{ fontSize: "0.72rem", fontWeight: 600 }}>Slot Weekday Off-Peak (07.00–16.00)</p>
                <p className="v3-muted" style={{ fontSize: "0.65rem" }}>Margin Rp530.000/jam</p>
              </div>
              <p className="v3-mono v3-gold" style={{ fontSize: "0.85rem", fontWeight: 700, flexShrink: 0, marginLeft: "0.5rem" }}>
                {hoursToBreakEvenWD} jam
              </p>
            </div>
            <div className="flex items-center justify-between" style={{ background: "rgba(201,162,39,0.08)", borderRadius: 8, padding: "0.45rem 0.65rem" }}>
              <div>
                <p style={{ fontSize: "0.72rem", fontWeight: 600 }}>Slot Weekend Off-Peak (07.00–16.00)</p>
                <p className="v3-muted" style={{ fontSize: "0.65rem" }}>Margin Rp650.000/jam</p>
              </div>
              <p className="v3-mono v3-gold" style={{ fontSize: "0.85rem", fontWeight: 700, flexShrink: 0, marginLeft: "0.5rem" }}>
                {hoursToBreakEvenWE} jam
              </p>
            </div>
          </div>
        </div>
      ) : (
        <div className="v3-surface" style={{ borderRadius: 14, padding: "0.8rem 1rem", borderLeft: "4px solid #4CAF61" }}>
          <p style={{ fontSize: "0.82rem", fontWeight: 600 }}>✅ Break-Even Tercapai</p>
          <p className="v3-muted" style={{ fontSize: "0.72rem", marginTop: "0.2rem" }}>
            Proyeksi sudah melampaui break-even. {gapHealthy > 0
              ? `Kekurangan ke target Sehat: ${formatRupiah(gapHealthy)} (${Math.ceil(gapHealthy / BREAKEVEN.marginBlended)} jam lagi).`
              : "Proyeksi sudah masuk zona KUAT — pertahankan momentum!"}
          </p>
        </div>
      )}

      {/* Utilization Tracker */}
      <div className="v3-surface" style={{ borderRadius: 14, padding: "1rem 1.1rem" }}>
        <div className="flex items-center justify-between" style={{ marginBottom: "0.5rem" }}>
          <p className="v3-display" style={{ fontSize: "0.85rem", fontWeight: 600 }}>📊 Utilization Rate</p>
          <p className="v3-mono v3-gold" style={{ fontSize: "0.9rem", fontWeight: 700 }}>{utilizationPct}%</p>
        </div>
        {hasDurationData ? (
          <>
            <div style={{ height: 8, background: "rgba(255,255,255,0.07)", borderRadius: 4, marginBottom: "0.4rem" }}>
              <div style={{ height: 8, width: utilizationPct + "%", background: utilizationPct >= 50 ? "#4CAF61" : utilizationPct >= 25 ? "#C9A227" : "#D1574A", borderRadius: 4, transition: "width 0.5s" }} />
            </div>
            <p className="v3-muted" style={{ fontSize: "0.68rem" }}>
              {hoursBooked} jam terisi dari {BREAKEVEN.totalHoursPerMonth} jam tersedia bulan ini
            </p>
          </>
        ) : (
          <p className="v3-muted" style={{ fontSize: "0.72rem" }}>
            Isi field "Durasi Bermain (Jam)" saat input transaksi Rental untuk mengaktifkan Utilization Tracker.
          </p>
        )}
        <div style={{ marginTop: "0.75rem", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "0.65rem" }}>
          <p style={{ fontSize: "0.72rem", fontWeight: 600, marginBottom: "0.4rem" }}>🎯 Slot Prioritas Isi</p>
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2" style={{ background: "rgba(209,87,74,0.1)", borderRadius: 7, padding: "0.35rem 0.6rem" }}>
              <span style={{ fontSize: "0.65rem", fontWeight: 700, color: "#D1574A" }}>P1</span>
              <p style={{ fontSize: "0.7rem" }}>Minggu 07.00–20.00 — secara historis paling sering kosong</p>
            </div>
            <div className="flex items-center gap-2" style={{ background: "rgba(201,162,39,0.1)", borderRadius: 7, padding: "0.35rem 0.6rem" }}>
              <span style={{ fontSize: "0.65rem", fontWeight: 700, color: "#C9A227" }}>P2</span>
              <p style={{ fontSize: "0.7rem" }}>Senin 23.00–00.00 — slot malam weekday jarang terisi</p>
            </div>
          </div>
        </div>
      </div>
...
function TransactionRow({ tx, highlighted, canEdit = true, onEdit, onDelete }) {
  ...
  const isPajakMasuk = tx.type === "pajak_masuk";

  const Icon = isTransfer ? ArrowLeftRight
    : isPrive ? Wallet
    : isPajakMasuk ? BarChart3
  ...
  const typeLabel = isTransfer ? `${tx.fromMethod} → ${tx.toMethod}`
    ...
    : isPajakMasuk ? `Pajak Masuk · ${tx.entity || ""}`
  ...
  const sign = isExpense || isPrive || isPiutangKeluar ? "-"
    : isIncome || isPiutangBalik || isPajakMasuk ? "+"
    : "";
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div className="flex items-center gap-1.5">
          ...
          {!isTransfer && !isPrive && !isPiutangKeluar && !isPiutangBalik && !isPajakMasuk && tx.status && tx.status !== "Lunas" && (
...
function TransactionModal({ editingTx, prefillData, existingPromos, currentBalances, allTransactions, onSave, onClose }) {
  ...
  const isPajakMasukType = type === "pajak_masuk";
  const splitsTotal = splits.reduce((sum, s) => sum + (parseFloat(s.amount) || 0), 0);
  const splitsValid =
    splits.length > 0 &&
    splits.every((s) => s.method && parseFloat(s.amount) > 0) &&
  ...
  const isValid =
    ...
    (type === "transfer"
      ? fromMethod !== toMethod
      : type === "prive" || isPiutangType || isPajakMasukType
      ? !!method
      : splitMode
      ? splitsValid
      : !!category && !!method);
  ...
  const handleSubmit = (e) => {
    ...
    else if (isPajakMasukType) tx = { ...base, method, entity: entity.trim() };
      ...
      <div className="v3-surface" style={{ borderRadius: 18, width: "100%", maxWidth: 440, maxHeight: "88vh", overflowY: "auto" }}>
        ...
        <form onSubmit={handleSubmit} style={{ padding: "1.1rem 1.2rem", display: "flex", flexDirection: "column", gap: "0.9rem" }}>
          <div className="grid grid-cols-3 gap-1.5">
            {[["income", "Income"], ["expense", "Expense"], ["transfer", "Transfer"], ["prive", "Prive"], ["pajak_masuk", "Pajak Masuk"], ["piutang_keluar", "Piutang Keluar"], ["piutang_balik", "Pengembalian"]].map(([val, label]) => (
              <button
                key={val}
                type="button"
                onClick={() => setType(val)}
          ...
          {type === "income" || type === "expense" ? (
            ...
            <>
              <p className="v3-muted" style={{ fontSize: "0.75rem", padding: "0.4rem 0.6rem", background: "rgba(77,127,176,0.1)", borderRadius: 8 }}>
                Uang pajak masuk ke saldo kantong tapi <strong>TIDAK dihitung sebagai income bisnis</strong> di laporan — dicatat terpisah sebagai kewajiban yang akan disetorkan ke pemerintah.
              </p>
              <Field label="Kantong Penerima">
                <select value={method} onChange={(e) => setMethod(e.target.value)} className="v3-input" style={{ borderRadius: 8, padding: "0.5rem 0.6rem", width: "100%", fontSize: "0.85rem" }}>
                  {METHODS.map((m) => <option key={m} value={m}>{m}</option>)}
                </select>
              </Field>
              <Field label="Dari Siapa / Keterangan (opsional)">
                <input
                  value={entity}
                  onChange={(e) => setEntity(e.target.value)}
                  placeholder="cth. Pajak Daerah 10% - PERTAMINA EP"
                  className="v3-input"
                  style={{ borderRadius: 8, padding: "0.5rem 0.6rem", width: "100%", fontSize: "0.85rem" }}
                />
              </Field>
